...
function Get-Json($url) { (Invoke-WebRequest -Uri $url -UseBasicParsing).Content | ConvertFrom-Json }
function Invoke-PostJson($url, $obj, $headers=@{}) {
  $json = $obj | ConvertTo-Json -Compress
  (Invoke-WebRequest -Uri $url -Method Post -Body $json -ContentType 'application/json' -Headers $headers -UseBasicParsing).Content | ConvertFrom-Json
}

$csrf = Get-Json "$BaseUrl/api/csrf.php"
...
$csv = Invoke-WebRequest -Uri "$BaseUrl/api/route_summary.csv.php" -UseBasicParsing
